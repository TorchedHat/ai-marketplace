...
async def parse_fusion_decisions(log_content: str) -> str:
    ...
    reasons = re.findall(r'(?:fuse|fusion).*?(?:because|reason):\s*(.+)', log_content, re.IGNORECASE)

    result = f"## Fusion Decisions Analysis\n\n"
    result += f"**Fused:** ~{fused} mentions\n"
    result += f"**Not fused:** ~{not_fused} mentions\n\n"

    if reasons:
        result += f"**Fusion reasons:**\n"
        for r in reasons[:10]:
            result += f"  - {r.strip()}\n"
...
async def parse_output_code(code_content: str) -> str:
    ...
    result = f"## Generated Kernel Analysis\n\n"
